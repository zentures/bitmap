...
func newRunningLengthWord(a []uint64, p int64) *runningLengthWord {
...
func (this *runningLengthWord) reset(a []uint64, p int64) *runningLengthWord {
	this.m = &(a[p])
	this.p = p
...
func (this *runningLengthWord) getActualWord() uint64 {
	return uint64(*this.m)
}
...
func (this *runningLengthWord) setNumberOfLiteralWords(n int64) {
	*this.m = uint64(*this.m) | NotRunningLengthPlusRunningBit
	*this.m = uint64(*this.m) & ((uint64(n) << uint64(RunningLengthBits + 1)) | RunningLengthPlusRunningBit)
}

// setRunningBit sets the running bit
func (this *runningLengthWord) setRunningBit(b bool) {
	if b {
		*this.m = uint64(*this.m) | 1
	} else {
		*this.m = uint64(*this.m) & ^uint64(1)
	}
}
...
func (this *runningLengthWord) setRunningLength(n int64) {
	...
	//fmt.Printf("setRunningLength before: %064b\n", this.a[this.p])
	*this.m = uint64(*this.m) | ShiftedLargestRunningLengthCount
	//fmt.Printf("setRunningLength shfitd: %064b\n", ShiftedLargestRunningLengthCount)
	//fmt.Printf("setRunningLength before: %064b\n", uint64(*this.m))
	*this.m = uint64(*this.m) & ((uint64(n) << 1) | NotShiftedLargestRunningLengthCount)
